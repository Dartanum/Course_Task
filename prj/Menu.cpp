...
Menu::Menu(settings& _setting, TopResults& records, textureLoader& textures_, Vector2u& ScreenSize, Font& font_, Font& recordFont, std::string Prefix) {
  prefix = Prefix;
  screenR = float(ScreenSize.x) / float(ScreenSize.y);
  textures = textures_.menu["mainTile"];
  logo.setSize(Vector2f(675 * screenR, 133 * screenR));
  ...
  font = font_;
  topFont = recordFont;
  buttonSize = Vector2f(107 * screenR, 30 * screenR);
  settings_ = _setting;
  top = records;
  delayFrame = 20;
  clickBuffer.loadFromFile(prefix + "/sounds/Menu/click1.ogg");
  rolloverBuffer.loadFromFile(prefix + "/sounds/Menu/rollover2.ogg");

  std::string line;
  std::ifstream reader(prefix + "/text/rules.ini");
  if (reader.is_open()) {
...
void Menu::buildMainMenu() {
  int interval = 10;
  int countBut = 5;
  std::string names[] = { "play", "rules", "records", "settings", "exit" };
...
void Menu::Rules(RenderWindow& window) {
  ...
  Sound rolloverSound(rolloverBuffer);
  Text ruless;
  ruless.setCharacterSize(12 * screenR);
  ruless.setFillColor(Color::Black);
  ruless.setFont(font);
  ruless.setString(sf::String(rules));
  MenuButton ButtonClose(textures, Vector2f(190, 50), Vector2f(0, 0), Vector2f(0, 49), buttonSize, "close", font);
...
void Menu::Settings(RenderWindow& window) {
  ...
  for (it = texts.begin(); it != texts.end(); it++) {
    ...
    it->second.setString(sf::String(it->first));
    it->second.setOrigin(it->second.getLocalBounds().width / 2, it->second.getLocalBounds().height / 2);
  }
  //›ÎÂÏÂÌÚ˚ ËÌÚÂÙÂÈÒ‡
  RectangleShape listBack(Vector2f(270 * screenR, 152 * screenR));
  Vector2f sizeBut = Vector2f(listBack.getSize().x / 4, listBack.getSize().y / 8);
  MenuButton ButtonClose(textures, Vector2f(190, 50), Vector2f(0, 0), Vector2f(0, 50), sizeBut, "close", font);
  MenuButton ButtonSave(textures, Vector2f(190, 50), Vector2f(0, 0), Vector2f(0, 50), sizeBut, "save", font);
  ...
  while (isOpen)
  {
    ...
    while (window.pollEvent(event)) {
      ...
      if (isEdit) {
        ...
        nickArea = buildNickArea(texts[nick]);
        oldNick = texts[nick];
      }
    ...
    if (ButtonClose.listen(window, clickSound, rolloverSound)) {
      ...
      newSettings.nickname = texts[nick].getString().toAnsiString();
...
int Menu::endGameMenu(RenderWindow& window, int pointsCount, int gameTime, RectangleShape& Background) {
  ...
  std::string str = "Name: " + settings_.nickname + '\n' + "points: " + std::to_string(pointsCount) + '\n' + "time: " + std::to_string(gameTime);
  text.setString(sf::String(str));
  text.setOrigin(text.getGlobalBounds().width / 2, text.getGlobalBounds().height / 2);
  MenuButton ButtonExit(textures, Vector2f(190, 50), Vector2f(0, 0), Vector2f(0, 50), buttonSize, "exit", font);
  MenuButton ButtonMenu(textures, Vector2f(190, 50), Vector2f(0, 0), Vector2f(0, 50), buttonSize, "menu", font);
  MenuButton ButtonRestart(textures, Vector2f(190, 50), Vector2f(0, 0), Vector2f(0, 50), buttonSize, "restart", font);
  int interval = 20;
