...
Vector2f Spawner::generatorPlanets(Vector2u screen, std::vector<Vector2f>& spawnPoints, int radius, int& area) {
  ...
  std::uniform_int_distribution<> rangeYDown(spawnPoints[area].y + radius, spawnPoints[area].y + (screen.y / 2 - radius - radius / 5));
  int x = rangeX(mersenne);
  int y;
  if (area % 2 == 1)
    y = rangeYDown(mersenne);
  else
    y = rangeY(mersenne);
  Vector2f result(x, y);
  return result;
...
float Spawner::generator(float min, float max) {
  int Min = min * 1000;
  int Max = max * 1000;
  std::uniform_int_distribution<> range(Min, Max);
  std::mt19937 mersenne(rd());
  int num = range(mersenne);
  float result = num / 1000.0;
  return result;
}
