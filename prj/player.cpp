...
Player::Player(Texture* texture, const double& speed, const double& angularSpeed, Vector2u Screen) : screen(Screen) {
  float screenCoeff = float(Screen.x) / float(Screen.y);
  player.setTexture(texture);
  player.setSize(Vector2f(63 * screenCoeff, 60 * screenCoeff));
  Player::speed = speed;
  Player::angularSpeed = angularSpeed;
  player.setOrigin(player.getSize().x / 2, player.getSize().y / 2);
  player.setPosition(Screen.x / 2, Screen.y / 2);
  anim_end = false;
  dest.setSize(Vector2f(player.getSize().x * 2, player.getSize().y * 2));
  dest.setOrigin(player.getOrigin());
}
...
void Player::Destroy(std::vector<Texture*>& textures) {
  ...
  for (ptrdiff_t i = 0; i < textures.size() * anim_speed; i += anim_speed) {
    ...
    if (currentFrame > i && currentFrame < i + anim_speed) {
      dest.setTexture(textures[i / (anim_speed * 2)]);
      break;
    }
  }
}