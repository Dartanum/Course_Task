...
Planet::Planet(float Radius, float AngularSpeed, Texture* texture, Vector2f Position, int hp_, float spawnKD_, int Area, Font& font, Texture* icon_, Texture* dest_effect)
	: radius(Radius), angularSpeed(AngularSpeed), position(Position), spawnKD(spawnKD_), hp(hp_), currentHp(hp_), occupiedArea(Area) {
	planet.setRadius(radius);
	texture_planet = *texture;
	dest_effect_planet = *dest_effect;
	planet.setTexture(&texture_planet);
	planet.setOrigin(Vector2f(radius, radius));
	planet.setPosition(position);
	planet.setScale(0, 0);
	deathTime = 0;
	icon.setSize(Vector2f(radius / 5, radius / 5));
...
void Planet::Die(float Radius, float AngularSpeed, Texture* Texture, Vector2f Position, int hp_, float spawnKD_, int area) {
	radius = Radius;
	planet.setRadius(radius);
	planet.setOrigin(Vector2f(radius, radius));
	planet.setPosition(Position);
...
void Planet::Transparency(RectangleShape& object) {
	if (Intersects(object) && born) {
		planet.setFillColor(Color(255, 255, 255, 100));
	}
	else planet.setFillColor(Color::White);
}

bool Planet::DestroyListener(int gameTime) {
	bool answer = (currentHp <= 0 && !destroy);
	if (answer) {
		deathTime = gameTime;
		destroy = true;
		dest_anim_clock.restart();
		born = false;
		texture_planet = dest_effect_planet;
	}
	return answer;
}